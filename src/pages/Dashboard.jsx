    ...
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <div ref={statsRef} className="grid grid-cols-3 gap-3 mb-8">
        {stats.map(({ label, value, icon: Icon, color }) => (
          <div
            key={label}
            className="glass group rounded-2xl p-4 text-center transition-all duration-200 hover:-translate-y-1 hover:bg-white/10 hover:shadow-lg hover:shadow-cyan-500/10"
          >
            <Icon
              className={`mx-auto mb-1 transition-transform duration-200 group-hover:scale-125 ${color}`}
              size={20}
            />
